...
"고객별 시술 히스토리 로그" 형식으로 **상세하고 체계적으로** 정리하는 것이다.



**🔴 핵심 규칙: 반드시 최소 5개 이상의 섹션을 생성하라. TRANSCRIPT가 짧아도 내용을 세분화하여 여러 섹션으로 나누어라. 🔴**
...
- **🔴 매우 중요: 전체 섹션 수는 최소 5개 이상, 가능하면 6-8개를 목표로 하라. 🔴**

- **🔴 TRANSCRIPT가 짧아도 언급된 내용을 최대한 세분화하여 여러 섹션에 나눠서 작성하라. 🔴**

- 각 섹션별 불릿 포인트는 1~3개를 기본으로 하되, **중요한 섹션(시술·관리 상세 등)은 3~5개까지 작성**하라.

- **정보가 부족해도 섹션을 생략하지 말고, TRANSCRIPT에서 추론 가능한 내용을 최대한 작성하라.**

- 예시:
  - "통증이 있었다" → "시술 후 상태·고객 반응" 섹션에 포함
  - "6주 후 방문" → "다음 방문·추천" 섹션에 포함
  - "쿨링팩 사용" → "주의사항·홈 케어" 섹션에 포함
  - "중간에 휴식" → "시술·관리 상세" 섹션에 포함

- 핵심 사실, 시술 내용, 특이사항, 다음 방문 관련 메모를 **각각 별도 섹션**으로 분리하라.

- **섹션을 줄이려 하지 말고, 최대한 상세하게 나누어라.**
...
- 반드시 **최소 5개 이상의 섹션**을 생성하라.

- TRANSCRIPT가 짧아도 언급된 내용을 세분화하여 여러 섹션에 나누어라.

- "내용이 없어서" 섹션을 생략하지 마라. 추론 가능한 내용이라도 작성하라.

- 각 섹션은 **최소 1개, 가능하면 2-3개의 구체적인 항목**을 포함하라.
...
- 고객 기본 정보:
  ...
  **이 섹션은 반드시 아래 순서와 형식으로 작성해야 한다. 다른 형식은 절대 사용하지 마라.**
  ...
  - 세 번째 줄: "성별: 여" 또는 "성별: 남" 처럼 성별만 단독으로 한 줄로 적는다.

    - TRANSCRIPT에서 "여성", "여자", "여자손님", "남성", "남자", "남자손님" 등의 표현에서 성별을 추출한다.

    - 성별을 알 수 없으면 "성별: 미기재"라고 적는다.

    - **중요: 성별은 이 줄에만 표시하고, 다른 곳(특히 고객 특징 줄)에는 절대 포함하지 마라.**

  - 네 번째 줄: "고객 특징: 20대 중반, 직장인, 곱슬 모발, 두피 민감"처럼

    대략적인 연령대·직업·모발/피부·성향 등을 한 줄로 요약한다.

    - **중요: 성별(여성, 남성, 여자, 남자, 여자손님, 남자손님 등)은 절대 포함하지 마라. 성별은 세 번째 줄에 별도로 표시되어 있다.**

    - TRANSCRIPT에 정보가 거의 없으면 "고객 특징: 미기재"라고 적는다.

  **JSON 출력 시 "고객 기본 정보" 섹션의 content 배열은 반드시 위 4줄을 순서대로 포함해야 한다.**
...
- 시술·관리 상세: **(필수 섹션 - 반드시 포함)**

  - 진행한 시술 단계, 부위, 사용 제품/약제(브랜드/모델명은 말한 경우만),

    컬러/호수, 도포 시간 등

  - **TRANSCRIPT에서 시술 관련 내용을 최대한 세분화하여 2-5개 항목으로 작성하라.**

  - 예: "- 브라질리언 + 종아리 왁싱, 평소보다 온도 약간 낮춰 진행"

  - 예: "- 첫 시술로 긴장도 높아 중간중간 설명 제공하며 진행"

  - 예: "- 통증 완화를 위해 중간에 2회 휴식 시간 제공"



- 시술 후 상태·고객 반응: **(권장 - 가능하면 포함)**

  - 만족도, 통증/붓기/트러블, 추가 요청/수정 사항

  - **시술 중이나 시술 후 고객의 반응, 피부 상태를 언급했다면 반드시 포함하라.**

  - 예: "- 통증은 평소와 비슷, 종아리 쪽은 깔끔하다고 만족 표현"

  - 예: "- 피부 예민하여 쿨링팩 사용 후 상태 안정"

  - 예: "- 통증 때문에 중간에 2회 휴식"



- 주의사항·홈 케어: **(권장 - 가능하면 포함)**

  - 피해야 할 행동, 샵에서 안내한 홈케어/제품(언급된 범위 내)

  - **쿨링팩, 진정 크림, 주의사항 안내 등이 언급되었다면 반드시 포함하라.**

  - 예: "- 24시간 사우나/온탕 금지, 샤워 시 미온수 사용 안내"

  - 예: "- 쿨링팩 및 진정 크림으로 사후 관리"

  - 예: "- 생리 주기 겹치지 않도록 일정 조절 필요"
...
- 다음 방문·추천: **(권장 - 가능하면 포함)**
  ...
  - **다음 방문 시기나 주의사항이 언급되었다면 반드시 포함하라.**

  - 예: "- 4주 후 리터치 권장, 인그로운 상태 보고 시술 강도 조절 예정"

  - 예: "- 6주 후 재방문 권장 (생리 주기 피하기)"

  - 예: "- 다음 시술 시 피부 상태 확인 후 진행"
...
TRANSCRIPT: "오늘 저녁에 8시 20분 박소영 고객님 처음 왁싱하러 오셨어. 브라질리언 첫 시술이라 긴장 많이 하셔서, 중간중간 설명 많이 해드리면서 진행했고, 통증 때문에 중간에 두 번 정도 쉬어가면서 했다. 피부가 살짝 예민해서 쿨링팩이랑 진정 크림 충분히 발라드림. 생리 주기 겹치지 말고 6주 후쯤 다시 오시라고 안내."
...

{
  ...
  "sections": [
    ...
    {
      "title": "방문·예약 정보",
      "content": ["2025년 12월 3일 (수) 20:20 방문"]
    },
    {
      "title": "시술·관리 상세",
      "content": [
        "브라질리언 왁싱 첫 시술 진행",
        "긴장 완화를 위해 중간중간 설명 제공하며 진행",
        "통증 완화를 위해 중간에 2회 휴식 시간 제공"
      ]
    },
    {
      "title": "시술 후 상태·고객 반응",
      "content": [
        "피부 예민하여 쿨링팩 및 진정 크림 충분히 사용",
        "시술 완료 후 상태 양호"
      ]
    },
    {
      ...
      "content": [
        "피부 예민 고객으로 사후 관리 필요",
        "쿨링팩 및 진정 크림으로 진정 처리 완료"
      ]
    },
    {
      "title": "다음 방문·추천",
      "content": [
        "생리 주기를 피하여 6주 후 재방문 권장",
        "다음 방문 시 피부 상태 확인 후 시술 진행"
      ]
    }
  ]
}
...
예시 2 - 기존 고객 (리터치, 7개 섹션):

TRANSCRIPT: "어, 오늘 3시 김민지 언니. 오늘 리터치 하러 왔는데 저번에 C컬 했더니 끝이 좀 처진다고 해서 이번엔 D컬로 바짝 올려달래. 그래서 D컬 11미리랑 12미리 섞어서 좀 화려하게 디자인 들어갔어. 눈물도 좀 많아서 글루는 예전에 썼던 강한 거 말고 순한 걸로 썼다. 결제는 저번에 끊어놓은 회원권에서 5만 원 차감했음."

예시 출력 (7개 섹션):

{
  "title": "속눈썹 D컬 리터치 및 디자인 강화",
  "customerInfo": { "name": "김민지", "phone": null },
  "recommendedTags": ["D컬", "속눈썹연장"],
  "sections": [
    {
      "title": "고객 기본 정보",
      "content": [
        ...
        "구분: 기존 고객",
        "성별: 여",
        "고객 특징: 속눈썹 리터치 고객, 눈물 많음"
      ]
    },
    {
      "title": "방문·예약 정보",
      "content": ["2025년 12월 3일 (수) 15:00 리터치 방문"]
    },
    {
      "title": "현재 상태·고객 고민",
      "content": ["이전 C컬 시술 후 끝이 처지는 느낌으로 더 바짝 올라가길 원함"]
    },
    {
      "title": "시술·관리 상세",
      "content": [
        "D컬 11mm + 12mm 섞어서 화려한 디자인으로 시술",
        "기존보다 컬을 강하게, 디자인 강조",
        "눈물이 많아 순한 글루 사용 (기존 강한 글루 대신)"
      ]
    },
    {
      "title": "시술 후 상태·고객 반응",
      "content": ["리터치 후 원하는 만큼 올라간 느낌으로 만족"]
    },
    {
      "title": "다음 방문·추천",
      "content": ["다음 리터치 시에도 D컬 유지 검토"]
    },
    {
      "title": "결제/매출",
      "content": ["기존 회원권에서 5만 원 차감"]
    },
    {
      "title": "참고 메모",
      "content": ["눈물이 많은 편이라 강한 글루 대신 순한 제품 사용 권장"]
    }
  ]
}

**🔴 중요: 위 예시처럼 TRANSCRIPT 내용을 최대한 세분화하여 최소 5-8개의 섹션을 만들어라. 🔴**

**이 예시들은 "형식과 구조"를 보여주는 참고이며, 실제 답변에서는 TRANSCRIPT와 {TODAY} 값에 맞게 새로 작성해야 한다.**
...

{
  ...
  "recommendedTags": ["오늘 실제로 시술한 내용의 태그만 배열로 나열"],
...
**중요: recommendedTags 필드 처리 규칙:**

- **오늘 실제로 시술한 내용만** 태그로 추천하라.
- **🔴 매우 중요: 시술명이 명시적으로 언급된 경우만 태그하라. 🔴**
- **간접적 표현이나 추론으로 태그하지 마라:**
  - "채워 넣기", "붙이기" → 속눈썹연장으로 추론하지 마라
  - "컬링", "웨이브" → 속눈썹펌으로 추론하지 마라
  - 반드시 "속눈썹연장", "속눈썹펌" 같은 **명시적 시술명**이 있어야 태그
- 과거 시술 내용이나 다음 예정 시술은 제외하라.
- 문맥을 이해해서 시간 표현을 구분하라:
  - "지난번에는 D컬, 오늘은 C컬" → ["C컬"]만 추천 (D컬 제외)
  - "이전에는 속눈썹펌, 이번에는 속눈썹연장" → ["속눈썹연장"]만 추천
- 태그 예시: ["C컬", "속눈썹연장", "젤네일", "브라질리언"]
- **명시적 시술명이 없으면 빈 배열 []로 반환하라.**
...
**중요: sections 배열 구성 규칙 (반드시 준수):**

1. **필수 섹션 (TRANSCRIPT 내용이 적어도 반드시 포함):**
   - "고객 기본 정보" (필수)
   - "시술·관리 상세" (필수)

2. **권장 섹션 (TRANSCRIPT에서 해당 내용을 찾거나 추론 가능하면 포함):**
   - "방문·예약 정보" (시간 정보가 있으면)
   - "현재 상태·고객 고민" (고객의 상태나 고민이 언급되면)
   - "시술 후 상태·고객 반응" (시술 후 반응이나 만족도가 언급되면)
   - "주의사항·홈 케어" (주의사항이나 홈케어 안내가 있으면)
   - "다음 방문·추천" (다음 방문 계획이나 권장사항이 있으면)
   - "결제/매출" (결제 관련 내용이 있으면)
   - "참고 메모" (특이사항이나 다음 시술자가 알아야 할 정보가 있으면)

3. **섹션을 최대한 상세하게 작성하라:**
   - TRANSCRIPT가 짧더라도 언급된 내용을 바탕으로 **최소 5개 이상의 섹션**을 만들어라.
   - 각 섹션의 content는 **최소 1개, 가능하면 2-3개의 항목**을 포함하라.
   - 정보가 부족해도 "미기재"라고 적지 말고, TRANSCRIPT에서 추론 가능한 내용을 최대한 작성하라.

**예시 JSON 구조:**

{
  "title": "브라질리언 왁싱 첫 시술 및 사후 관리 안내",
  "customerInfo": { "name": "박소영", "phone": null },
  "recommendedTags": ["브라질리언"],
  "sections": [
    {
      "title": "고객 기본 정보",
...
- "title"에는 고객 이름을 절대 포함하지 말고, 순수 시술 행위만 요약할 것.

- **🔴 "sections"는 반드시 최소 5개 이상, 가능하면 6-8개를 작성하라. 🔴**

- **🔴 TRANSCRIPT가 짧아도 내용을 세분화하여 섹션을 나누고, 각 섹션마다 1-3개의 구체적인 항목을 포함하라. 🔴**

- 섹션 예시 (이 중 최소 5개 이상 포함):
  1. "고객 기본 정보" (필수)
  2. "방문·예약 정보" (시간이 있으면)
  3. "현재 상태·고객 고민" (상태 언급 시)
  4. "시술·관리 상세" (필수)
  5. "시술 후 상태·고객 반응" (반응 언급 시)
  6. "주의사항·홈 케어" (주의사항 언급 시)
  7. "다음 방문·추천" (다음 계획 언급 시)
  8. "결제/매출" (결제 언급 시)
  9. "참고 메모" (특이사항 언급 시)
